...
Deno.serve(async (req) => {
  ...
  if (externalEventId) {
    const { data: duplicate } = await db.from("crm_external_events").select("id,processed").eq("provider", provider).eq("external_event_id", String(externalEventId)).maybeSingle();
  ...
  const valueRaw = pick(payload, ["value", "amount", "price", "total"]);
  ...
  if (!contact) {
    const { data, error } = await db.from("crm_contacts").insert({
      ...
      tags: inferredTags,
      external_id: String(pick(source, ["subscriber_id", "contact_id", "customer_id"]) || "") || null,
    }).select("*").single();
  ...
  const { data: existingOpp } = await db.from("crm_opportunities")
    .select("*")
  ...
  if (opportunity) {
    ...
    const { data, error } = await db.from("crm_opportunities").insert({
      ...
      value,
      stage,
      loss_reason: eventType.includes("abandon") ? "Carrinho abandonado" : null,
    }).select("*").single();
    if (error) return json({ error: error.message }, 500);
    opportunity = data;
  }

  const { data: evt, error: evtError } = await db.from("crm_external_events").insert({
    provider,
  ...
  if (stage === "recuperacao") {
    await db.from("crm_activities").insert({
      ...
      notes: `Evento recebido de ${provider}`,
